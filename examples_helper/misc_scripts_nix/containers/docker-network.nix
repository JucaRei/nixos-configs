...
with lib; let
  ...
  dockerNetworkOpts = _: {
    options = {
      ...
      opts = mkOption {type = with types; attrsOf str;};
    };
  };
in {
  ...
  config = mkIf (eachNetwork != {}) {
    systemd.services =
      mapAttrs'
      (netName: netCfg: (nameValuePair "docker-network-${netName}-init" (mkIf netCfg.enable {
        ...
        script = let
          optsList =
            mapAttrsToList (name: value: "--opt ${name}=${value}") netCfg.opts;
