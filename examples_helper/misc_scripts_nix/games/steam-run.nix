{ config, lib, pkgs, ... }:
with lib;
let cfg = config.nixfiles.modules.games.steam-run;
in {
          ...
          extraLibraries = _:
            ...
            optionals mountandblade [
              (glew.overrideAttrs (_: super:
                let opname = super.pname;
                in rec {
                  pname = "${opname}-mountandblade";
                  inherit (super) version;
                  src = fetchurl {
                    url =
                      "mirror://sourceforge/${opname}/${opname}-${version}.tgz";
                    sha256 =
                      "sha256-BN6R5+Z2MDm8EZQAlc2cf4gLq6ghlqd2X3J6wFqZPJU=";
                  };
                }))
