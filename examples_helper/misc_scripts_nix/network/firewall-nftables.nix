...
let
  verifiedNetfilter = { text, modules ? [ ], }:
    let
      ...
      check = vmTools.runInLinuxVM (pkgs.runCommand "nft-check"
        {
  ...
  ruleset =
    let
      ...
      makePortRanges = portRanges: protocol:
        lib.concatMapStringsSep "\n"
          (pr: "${protocol} dport ${toString pr.from}-${toString pr.to} accept")
          portRanges;
      ...
      trashFile = pkgs.writeText "nftables-trashfile" ''
        ...
        connectionTrackingModules DEPRECATED: ${
          lib.generators.toJSON { } cfg.connectionTrackingModules
        }
      '';
    in
    ''
      ...
      table inet filter {
        chain input {
          ...
          # allow ICMP
          ${if cfg.allowPing then allowPingIPv4Text else ""}
          ${if cfg.allowPing then allowPingIPv6Text else ""}
          ...
          ${
            lib.concatMapStrings (name: ''
              iifname ${name} jump interface-${name}
...
in
{
  ...
  options.networking.firewall = {
    ...
    checkReversePath = mkOption {
      ...
      default = false;
      apply = x:
        trace "nftables.firewall: checkReversePath value ${
          lib.generators.toJSON { } x
        }"
          x;
    };
    extraCommands = mkOption {
      ...
      apply = x:
        trace
          "nftables.firewall: extraCommands value ${lib.generators.toJSON { } x}"
          x;
    };
    extraStopCommands = mkOption {
      ...
      default = "";
      apply = x:
        trace "nftables.firewall: extraStopCommands value ${
          lib.generators.toJSON { } x
        }"
          x;
    };
    ...
    connectionTrackingModules = mkOption {
      ...
      default = [ ];
      apply = x:
        trace "nftables.firewall DEPRECATED: connectionTrackingModules value ${
          lib.generators.toJSON { } x
        }"
          x;
    };
