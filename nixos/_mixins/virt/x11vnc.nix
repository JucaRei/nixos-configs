...

{
  config = lib.mkIf config.services.xserver.enable {
    ...
    networking.firewall.allowedTCPPorts =
      [ 5900 ]; # Since x11vpn defaults to port 5900. Open this port in firewall

    environment = {
      # VNC used for remote access to the desktop
      systemPackages = with pkgs; [ x11vnc ];
    };
    ...
    systemd.services."x11vnc" = {
      ...
      serviceConfig = {
        ...
        ExecStart =
          "${pkgs.x11vnc}/bin/x11vnc -passwdfile /home/${username}/passwd -noxdamage -nap -many -repeat -clear_keys -capslock -xkb -forever -loop100 -auth /var/run/lightdm/root/:0 -display :0 -clip 1920x1080+1920+0";
        #ExecStart = "${pkgs.x11vnc}/bin/x11vnc -passwdfile /home/${username}/passwd -noxdamage -nap -many -repeat -clear_keys -capslock -xkb -forever -loop100 -auth /var/run/lightdm/root/:0 -display :0";
